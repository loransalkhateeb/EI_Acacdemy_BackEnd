...
const xssRegex =
  /<script.*?>|<\/script>|<img.*?>|onerror|onload|javascript:|data:/i;
  ...
  const schema = Joi.object({
    status: Joi.string()
      ...
      .custom((value, helpers) => {
        if (xssRegex.test(value)) {
          return helpers.error("string.pattern.base", {
            message:
              "Input contains potentially malicious content like <script> or <img> tags.",
          });
        }
        const sqlInjectionRegex = /SELECT|INSERT|UPDATE|DELETE|DROP|--|\*/i;
        if (sqlInjectionRegex.test(value)) {
          return helpers.error("string.pattern.base", {
            message: "Input contains potentially harmful SQL characters.",
          });
        }
        return value;
      })
      .messages({
        ...
        "string.pattern.base":
          "Input contains potentially malicious content or SQL injection risk.",
      }),
    book_name: Joi.string().min(3).max(255).required(),
